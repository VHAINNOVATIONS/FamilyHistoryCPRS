...
type
  ...
  private
    ...
    function BuildDiseaseCondMaster:TStringList;
    procedure LoadFilteredDiseaseList(aList:TStrings);
...
procedure TfrmFamHistory.LoadFilteredDiseaseList(aList:TStrings);
var
  disGrp, disType, GrpType, GrpIEN, TypeIEN,
  X, holdName, holdRelat, holdVStatus, holdCond: string;
  i,ii,f: Integer;
  tmpList,aDiseases,aFilters: TStrings;
  ADiseaseCondMaster:TStringList;

begin
  ADiseaseCondMaster := TStringList.Create;
  ADiseaseCondMaster.Clear;
  ADiseaseCondMaster.Assign(BuildDiseaseCondMaster);
  tmpList := TStringList.Create;
  aDiseases := TStringList.Create;
  ...
  aFilters.Assign(aList);
  FastAssign(GetAllFamilyHistoryRecords(Patient.DFN), tmpList);


  try
    begin
      for ii := 0 to ADiseaseCondMaster.Count - 1 do
          begin
           x := ADiseaseCondMaster[ii];
          end;
...
function TfrmFamHistory.BuildDiseaseCondMaster:TStringList;
var
